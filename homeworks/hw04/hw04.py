...
def get_num_leaves(t):
    if t.is_leaf():
        return 1
    else:
        leaves_count = 0
        for b in t.branches:
            leaves_count += get_num_leaves(b)
        return leaves_count

    return get_num_leaves(t)

def has_path(t, x):
    if t.label == x:
        return True
    else:
        truth_cumulator = False
        for b in t.branches:
            truth_cumulator = truth_cumulator or (b.label == x or has_path(b, x)) 
        return truth_cumulator
